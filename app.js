...
function exportarExcel() {
  try {
    ...
    const r1  = DB.getEquipos().map(e => {
      ...
      return [e.nombre, e.serial, e.cat, e.cant, d, est, e.tarifa || 0, e.mantFecha || ''];
    });
    ...
    const r2  = DB.getEventos().map(e => [
      e.nombre, e.cliente, e.inicio, e.fin, e.estado,
      (e.equiposCant || []).map(x => { const eq = DB.getEquipo(x.id); return eq ? `${eq.nombre} x${x.cant}` : ''; }).filter(Boolean).join(', '),
      (e.personal    || []).map(id => DB.getPersona(id)?.nombre || '').filter(Boolean).join(', '),
      e.notas,
    ]);
    ...
    const r3  = DB.getCotizaciones().map(c => [c.nombre, c.cliente, c.fecha, c.dias, c.estado, c.total]);
    ...
    const r4  = DB.getClientes().map(c => [c.nombre, c.empresa, c.tel, c.email, c.ciudad, c.tipo]);
