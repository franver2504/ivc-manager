...
function exportarExcel() {
  try {
    ...
    const s = v => { if (v === null || v === undefined) return ''; if (typeof v === 'object') return JSON.stringify(v); return String(v); };
    const n = v => { const x = parseFloat(v); return isNaN(x) ? 0 : x; };

    // Inventario
    const ws1 = XLSX.utils.aoa_to_sheet([
      ['Equipo','Serial','Categoria','Total','Disponibles','Estado','Tarifa','Mantenimiento'],
      ...DB.getEquipos().map(e => {
        const d = Calc.disponibles(e);
        const est = e.estado === 'Mantenimiento' ? 'Mantenimiento' : d === 0 ? 'En evento' : d < e.cant ? 'Parcial' : 'Disponible';
        return [s(e.nombre), s(e.serial), s(e.cat), n(e.cant), n(d), s(est), n(e.tarifa), s(e.mantFecha)];
      })
    ]);
    ...
    const ws2 = XLSX.utils.aoa_to_sheet([
      ['Evento','Cliente','Inicio','Fin','Estado','Notas'],
      ...DB.getEventos().map(e => [s(e.nombre), s(e.cliente), s(e.inicio), s(e.fin), s(e.estado), s(e.notas)])
    ]);
    XLSX.utils.book_append_sheet(wb, ws2, 'Eventos');

    // Cotizaciones
    const ws3 = XLSX.utils.aoa_to_sheet([
      ['Cotizacion','Cliente','Fecha','Dias','Estado','Total'],
      ...DB.getCotizaciones().map(c => [s(c.nombre), s(c.cliente), s(c.fecha), n(c.dias), s(c.estado), n(c.total)])
    ]);
    XLSX.utils.book_append_sheet(wb, ws3, 'Cotizaciones');

    // Clientes
    const ws4 = XLSX.utils.aoa_to_sheet([
      ['Nombre','Empresa','Telefono','Email','Ciudad','Tipo'],
      ...DB.getClientes().map(c => [s(c.nombre), s(c.empresa), s(c.tel), s(c.email), s(c.ciudad), s(c.tipo)])
    ]);
    XLSX.utils.book_append_sheet(wb, ws4, 'Clientes');

    XLSX.writeFile(wb, 'IVC_Manager.xlsx');
    Toast.ok('Exportacion completada');
  } catch (err) {
